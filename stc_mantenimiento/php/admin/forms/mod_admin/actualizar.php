<?php
	...
	$consulta=ConsultarProducto($_GET['id_administrador']);

  function ConsultarProducto($id_admin){

	require ('../../../../conexiones/conexion_0.php');

 	$sentencia = mysqli_query($link, "SELECT * FROM  administrador Where id_administrador = '".$id_admin."' ");
	$filas= mysqli_fetch_array($sentencia);

    return [
    	$filas['id_administrador'],
	    $filas['no_expediente'],
	    $filas['nom_administrador'],
	    $filas['apellidos_administrador'],
	    $filas['nom_linea'],
	    $filas['nom_estacion'],
	    $filas['id_estacion'],
	    $filas['nom_area'],
...
<!--Importamos las librerias de jquery para poder utilizar sus animaciones y funciones-->
	...
	<body>
		...
		<div id= "actualiza" align="center">
			...
			<form action="act_datos_admin.php" method="POST">
				...
				<input id = "expe" type="text" name="no_expediente" maxlength=15 
								autocomplete="off" placeholder="No. De Expediente" 
								value="<?php echo $consulta[1] ?>" required><br>
				<label>Nombre: </label>
				<input id = "name" type="text" name="nom_administrador" maxlength=20 
								autocomplete="off" placeholder="Nombre(s)" 
								value="<?php echo $consulta[2] ?>" required><br>
				<label>Apellidos: </label>
				<input id = "apell" type="text" name="apellidos_administrador" maxlength=35 
